...
if "font_size" not in st.session_state:
    st.session_state.font_size = 18  # الحجم الافتراضي
...
def highlight_keywords(text, keywords, normalized_keywords=None, exact_match=False):
    # تمييز الكلمات المطابقة تطابق تام (mark أصفر)
    for kw in keywords:
        if not kw: continue
        pattern = re.compile(r"(?<!\w)"+re.escape(kw)+r"(?!\w)", re.IGNORECASE)
        text = pattern.sub(r'<mark>\g<0></mark>', text)
    # تمييز الكلمات المشابهة (غير مطابقة تطابق تام) بلون مختلف
    if exact_match and normalized_keywords:
        ...
        for i, norm_kw in enumerate(normalized_keywords):
            ...
            # إذا لم يتم تطابق تام، لكن الكلمة موجودة بشكل مرن
...
def render_law_file_viewer(files):
    ...
    if st.session_state.get("show_law_modal", False):
...
def run_main_app():
    ...
    with st.sidebar:
        ...
        with col_a:
            if st.button("➖", key="font_decrease") and st.session_state.font_size > 14:
                st.session_state.font_size -= 2
        with col_b:
            st.markdown(f"<div style='text-align:center;font-size:18px;'>{st.session_state.font_size}px</div>", unsafe_allow_html=True)
        with col_c:
            if st.button("➕", key="font_increase") and st.session_state.font_size < 32:
                st.session_state.font_size += 2
    ...
    p, div, textarea, input, .stTextInput, .stTextArea, .stExpanderHeader, .result-box-night {{
        font-size: {st.session_state.font_size}px !important;
    }}
    .stButton button, .stDownloadButton button, .copy-material-btn {{
        font-size: {max(16, st.session_state.font_size-2)}px !important;
    }}
    </style>
    """, unsafe_allow_html=True)

    # ================ CSS الوضع الليلي وتلوين النتائج ================
