...
def run_main_app():
    ...
    with st.form("main_search_form"):
        ...
        # مربع رقم المادة
        st.markdown('<div style="direction: rtl; text-align: right;">🔢 رقم المادة:</div>', unsafe_allow_html=True)
        ...
        if results:
            ...
            for i, r in enumerate(filtered):
                with st.expander(f"📚 المادة ({r['num']}) من قانون {r['law']}", expanded=True):
                    st.markdown(f'''
                    <div style="background-color:#f1f8e9;padding:20px;margin-bottom:10px;max-width:100%;border-radius:10px;
...
def main():
    ...
    st.markdown(
        """
        <div style="display: flex; flex-direction: column; align-items: center; margin-bottom: 32px; margin-top: 24px;">
            <!-- رمز الميزان SVG -->
            ...
            <div style="color: #2e7d32; font-size: 2.4rem; font-family: 'Cairo', 'Tajawal', sans-serif; font-weight: 800; text-align: center;">
    ...
    st.markdown("<div style='text-align:center; color:#2e7d32; font-size:22px; font-weight:bold; padding:20px;'>مرحبا بك عزيزي المستخدم، قم بالنقر على أيقونة التجربة المجانية لتصفح التطبيق واستخدامه بشكل كامل قبل طلب النسخة المدفوعة.</div>", unsafe_allow_html=True)

    with st.container(border=True):
        st.markdown("<h3 style='text-align:center; color:#2e7d32;'>🔐 النسخة المدفوعة</h3>", unsafe_allow_html=True)
        code = st.text_input("أدخل كود التفعيل:", key="activation_code_input", help="أدخل الكود الذي حصلت عليه لتفعيل النسخة الكاملة.")
    ...
    with st.container(border=True):
        ...
        if trial_start is None:
            if st.button("🚀 بدء التجربة المجانية (3 دقائق)", key="start_trial_button", use_container_width=True):
                ...
                st.success("✅ بدأت النسخة التجريبية الآن. لديك 3 أيام لتجربة التطبيق.")
                run_main_app()
                st.stop()
