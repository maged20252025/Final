...
# ------------------ ستايل الكتابة من اليمين + النتائج ------------------
st.markdown(f"""
<style>
mark {{
...
.result-box-custom {{
    ...
    direction: rtl;
    text-align: right;
    font-size: {st.session_state.results_font_size}px;
    transition: font-size 0.2s;
}}
/* كل مربعات الإدخال والكتابة من اليمين */
textarea, .stTextArea textarea, .stTextInput input, input[type="text"] {{
...
with tabs[0]:
    ...
    # رابط واتساب في الأعلى
    st.markdown(
        """
        <div style='text-align:left;'>
        <a href="https://wa.me/967777533034?text=مرحبًا، لدي استفسار بخصوص تطبيق القوانين اليمنية." target="_blank" style="display:inline-block;text-align:center;">
            ...
            <span style="font-size:13px;">مراسلتنا عبر واتساب</span>
        </a>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    if not os.path.exists(LAWS_DIR):
        ...
        if not files:
            ...
            if law_sel:
                ...
                st.markdown("""
                <style>
                textarea#full_law_view_text, .stTextArea textarea#full_law_view_text {
                    direction: rtl !important;
                    text-align: right !important;
                }
                </style>
