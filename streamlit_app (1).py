...
# ----------------------------------------------------
def run_main_app():
    # إضافة CSS لتصحيح اتجاه مربع النص وزر التصدير والعداد + تحسين زر نسخ المادة
    components.html("""
    <style>
    .scroll-btn {
        position: fixed;
    ...
    /* تحسين زر النسخ: ايقونة دائرية وأنيقة */
    .modern-copy-btn {
        display: inline-flex;
        align-items: center;
        gap: 10px;
    ...
    .modern-copy-btn .copy-circle {
        width: 34px;
        height: 34px;
        border-radius: 50%;
        background: linear-gradient(135deg, #fff 60%, #AED581 100%);
        display: flex;
        align-items: center;
        justify-content: center;
        font-size: 20px;
        box-shadow: 0 1px 4px #388e3c55;
        margin-left: 10px;
        transition: background 0.2s;
    }
    .modern-copy-btn .copied-check {
        font-size: 22px;
        color: #ffd600;
        margin-right: 8px;
        display: none;
    }
    .modern-copy-btn.copied .copied-check {
        display: inline;
        animation: fadein-check 1s;
    }
    .modern-copy-btn.copied .copy-circle {
        background: linear-gradient(135deg, #AED581 60%, #43cea2 100%);
    }
    ...
    with st.form("main_search_form"):
        ...
        # مربع البحث يدعم اتجاه RTL تلقائياً عبر CSS (تم تعديله - كذلك المؤشر/caret)
        keywords_form = st.text_area(
            ...
            placeholder="مثال: محكمة الأموال العامة"
        )
        ...
        if results:
            ...
            for i, r in enumerate(filtered):
                with st.expander(f"📚 المادة ({r['num']}) من قانون {r['law']}", expanded=True):
                    ...
                    ''', unsafe_allow_html=True)
                    # زر نسخ المادة: تصميم جديد عصري ودائري واحترافي
                    components.html(f"""
                        <button class="modern-copy-btn" id="copy_btn_{i}_{r['law']}_{r['num']}" onclick="
