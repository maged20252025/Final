...
def run_main_app():
    ...
    with st.form("main_search_form"):
        ...
        # ----------- تعديل اتجاه الكتابة في مربع البحث -----------
        keywords_form = st.text_area(
            ...
            # No need to set direction here, handled by CSS above!
        )
...
# ----------------------------------------------------
def main():
    # ---------- هيدر نصي مع رمز الميزان الاحترافي ----------
    st.markdown(
        """
    ...
    with st.container(border=True):
        ...
        if trial_start is not None:
            # ----------- مربع أزرق مع عداد متحرك متوسط الشاشة -----------
            ...
            if remaining_time > 0:
                # animated timer using streamlit_autorefresh
                from streamlit_autorefresh import st_autorefresh
                count = st_autorefresh(interval=1000, limit=None, key="trial_timer_autorefresh")
                # calculate time
