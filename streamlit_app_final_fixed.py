...
with st.form("main_search_form"):
    ...
    # تنظيف أسماء القوانين من .docx
    cleaned_files = [f.replace(".docx", "") for f in files]

    # إنشاء قائمة بفواصل بين كل قانون (خط وهمي ---)
    laws_with_separators = []
    for name in cleaned_files:
        laws_with_separators.append(name)
        laws_with_separators.append("---")
    if laws_with_separators and laws_with_separators[-1] == "---":
        laws_with_separators = laws_with_separators[:-1]

    # إضافة خيار "الكل" في أعلى القائمة
    display_files = ["الكل"] + laws_with_separators

    # تعديل التنسيق CSS للقائمة
    st.markdown("""
        <style>
    ...
    # عرض قائمة القوانين مع الخيارات المعدلة
    selected_display_name = st.selectbox("", display_files, key="main_file_select", label_visibility="collapsed")

    # إعادة ربط الاسم الحقيقي للملف بناءً على الاختيار (إذا لم يكن الكل أو فاصل)
    if selected_display_name == "الكل" or selected_display_name == "---":
        selected_file_form = selected_display_name
    else:
        selected_file_form = selected_display_name + ".docx"

    # عنوان الحقل الثاني: البحث بالكلمات
